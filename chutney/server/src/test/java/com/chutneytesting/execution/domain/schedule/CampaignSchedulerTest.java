...
public class CampaignSchedulerTest {
    ...
    void should_reschedule_missed_campaign_with_frequency_from_now_in_the_future() {
        PeriodicScheduledCampaign scheduledCampaignWeeklyFrequency = new PeriodicScheduledCampaign(1L, LocalDateTime.now().minusWeeks(5).minusHours(1).plusMinutes(15).withSecond(0).withNano(0), Frequency.WEEKLY, null, of(new CampaignExecutionRequest(11L, "campaign title 1", null)));
        PeriodicScheduledCampaign scheduledCampaignHourlyFrequency = new PeriodicScheduledCampaign(2L, LocalDateTime.now().minusHours(6).plusMinutes(15).withSecond(0).withNano(0), Frequency.HOURLY, null, of(new CampaignExecutionRequest(22L, "campaign title 2", null)));
        PeriodicScheduledCampaign scheduledCampaignNoFrequency = new PeriodicScheduledCampaign(3L, LocalDateTime.now().minusYears(1).minusHours(6).plusMinutes(15).withSecond(0).withNano(0), EMPTY, null, of(new CampaignExecutionRequest(33L, "campaign title 3", null)));
        when(scheduledCampaignRepository.getAll())
            .thenReturn(
                of(scheduledCampaignWeeklyFrequency, scheduledCampaignHourlyFrequency, scheduledCampaignNoFrequency)
            );
        when(scheduledCampaignRepository.add(any()))
            .thenReturn(
                null
            );
        doNothing().when(scheduledCampaignRepository).removeById(any());

        PeriodicScheduledCampaign expected1 = new PeriodicScheduledCampaign(1L, LocalDateTime.now().plusWeeks(1).minusHours(1).plusMinutes(15).withSecond(0).withNano(0), Frequency.WEEKLY, null, of(new CampaignExecutionRequest(11L, "campaign title 1", null)));
        PeriodicScheduledCampaign expected2 = new PeriodicScheduledCampaign(2L, LocalDateTime.now().plusMinutes(15).withSecond(0).withNano(0), Frequency.HOURLY, null, of(new CampaignExecutionRequest(22L, "campaign title 2", null)));

        // WHEN
        sut.scheduledMissedCampaignIds();

        // THEN
        verify(scheduledCampaignRepository).add(expected1);
        verify(scheduledCampaignRepository).add(expected2);
        verify(scheduledCampaignRepository).removeById(1L);
        verify(scheduledCampaignRepository).removeById(2L);
        verify(scheduledCampaignRepository).removeById(3L);
    }
    ...
    void should_execute_sequentially_campaigns() {
        PeriodicScheduledCampaign sc1 = new PeriodicScheduledCampaign(1L, now(clock).minusSeconds(5), Frequency.HOURLY, environment,  of(new CampaignExecutionRequest(11L, "campaign title 1", null), new CampaignExecutionRequest(22L, "campaign title 2", null)));
